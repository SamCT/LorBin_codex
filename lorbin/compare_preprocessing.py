...
# Keep these aligned with LorBin defaults so this tool compares default behavior only.
DEFAULT_EDGE = 75
...
def run_compare(fasta, output_dir, bam=None, depth_bga=None):
    ...
    old_kmer, old_len = old_generate_kmer_features_from_fasta(
        fasta,
        length_threshold=DEFAULT_LENGTH_THRESHOLD,
        kmer_len=DEFAULT_KMER_LEN,
    )
    new_kmer, new_len = generate_kmer_features_from_fasta(
        fasta,
        length_threshold=DEFAULT_LENGTH_THRESHOLD,
        kmer_len=DEFAULT_KMER_LEN,
    )
    ...
    old_cov = old_calculate_coverage(
        iter(depth_lines),
        bam_label,
        edge=DEFAULT_EDGE,
        contig_threshold=DEFAULT_CONTIG_THRESHOLD,
    )
    new_cov = calculate_coverage(
        iter(depth_lines),
        bam_label,
        edge=DEFAULT_EDGE,
        contig_threshold=DEFAULT_CONTIG_THRESHOLD,
    ...
    summary = {
        "comparison_mode": "default_lorbin_preprocessing",
        "parameters": {
            "fasta": str(fasta),
            "bam": bam,
            "depth_bga": depth_bga,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare default pre-optimization and optimized LorBin preprocessing outputs on real inputs."
    )
